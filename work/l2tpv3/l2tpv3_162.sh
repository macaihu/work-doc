...
default_conf()
{
    ...
    peer_ip=192.168.1.162
    local_session_id=1010
    peer_session_id=1020
    mtu=1476
    local_port=5000
    peer_port=6000
    local_tun_ip=10.6.7.1/24
    peer_tun_ip=10.6.7.2/24
    default_gw_enable=0
    bridge_enable=0
}
...
setup_tun_ip()
{
    ...
    ip link set l2tpeth1 up mtu ${mtu} 2>/dev/null
    ip addr add ${local_tun_ip} dev l2tpeth1 2>/dev/null
...
add_default_route()
{
    ...
    [ "${default_gw_enable}" = "1" ] && {
        ...
        if [ "${cur_def_route}" != "" ]; then
            ...
            ip route change default via ${peer_tun_ip} dev l2tpeth1 2>/dev/null
        else
            ip route add default via ${peer_tun_ip} dev l2tpeth1 2>/dev/null
        fi    
    }
}
...
del_default_route()
{
    cur_def_route=$(ip route |grep default |grep l2tpeth1)
...
{
    [ "${bridge_enable}" = "1" ] && {
        brctl addif br0 l2tpeth1 2>/dev/null
    }
}
